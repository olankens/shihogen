...
class Alauncher(machine: Shield, context: Context) : Updater(machine, context) {
    ...
    suspend fun setApplicationByIndex(payload: String, section: Int, adapted: Boolean = true) =
        withContext(IO) {
            machine.runEscape()
            machine.setLocale(DeviceLocale.EN_US)
            runRevealSettings()
    ...
    suspend fun setApplicationByTitle(payload: String, section: String, adapted: Boolean = true) =
        withContext(IO) {
            machine.runEscape()
            machine.setLocale(DeviceLocale.EN_US)
            runRevealSettings()
