...
abstract class Device(
    ...
    suspend fun runAttach() = withContext(IO) {
        ...
        manager = Kadb.create(address, port, connectTimeout = 10000, socketTimeout = 10000).also {
    ...
    suspend fun runKeygen() = withContext(IO) {
        ...
        //     val pair = KadbCert.rotate()
        //     sharing.edit()
        //         .putString("cert", pair.certificatePem.decodeToString())
        //         .putString("key", pair.privateKeyPem.decodeToString()).apply()
        //     return@withContext
        // }
        // try {
        //     KadbCert.importIdentity(key.toByteArray(), cert.toByteArray())
        // } catch (e: Exception) {
        //     if (e is CertificateExpiredException || e is CertificateNotYetValidException) {
        //         val pair = KadbCert.rotate()
        //         sharing.edit().putString("cert", pair.certificatePem.decodeToString())
        //             .putString("key", pair.privateKeyPem.decodeToString()).apply()
        //     }
        //     throw e
        // }
        KadbCert.configure(
            store = OkioFilePrivateKeyStore(
                privateKeyPath = context.filesDir.resolve("kadb_private_key.pem").path.toPath()
            ),
    ...
    suspend fun runLocate(pattern: String): Point? = withContext(IO) {
        ...
        val matches =
            content.let { Regex("[0-9]+").findAll(it.toString()).map(MatchResult::value).toList() }
    ...
    suspend fun runScrape(pattern: String): Node? = withContext(IO) {
        ...
        while (element == null) {
            ...
            val factors =
                factory.newXPath().evaluate(pattern, scraped, XPathConstants.NODESET) as NodeList
